...
add-apt-repository -y 'deb [arch=amd64,i386] http://mirrors.hustunique.com/mariadb/repo/10.1/ubuntu xenial main'
apt-get update > /dev/null

apt-get install mariadb-server-10.0 -y > /dev/null

echo "Configure MariaDB"
mysql -p -e "CREATE DATABASE symfony; CREATE USER 'symfony'@'%' IDENTIFIED BY 'symfony'; GRANT ALL PRIVILEGES ON symfony.* TO 'symfony'@'%'; FLUSH PRIVILEGES;"
